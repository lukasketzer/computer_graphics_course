...
window.onload = async () => {
    ...
    let verteciesPlane = [
        vec4(-2, -1 - 0.00001, -1, 1),
        vec4(2, -1 - 0.00001, -1, 1),
        vec4(-2, -1 - 0.00001, -5, 1),
        vec4(2, -1 - 0.00001, -1, 1),
        vec4(2, -1 - 0.00001, -5, 1),
        vec4(-2, -1 - 0.00001, -5, 1)
    ];
    ...
    gl.bufferData(gl.ARRAY_BUFFER, flatten(verteciesPlane.concat(verteciesQuad1).concat(verteciesQuad2)), gl.STATIC_DRAW);
    ...
    gl.bufferData(gl.ARRAY_BUFFER, flatten(textureCoordinates.concat(textureCoordinates).concat(textureCoordinates)), gl.STATIC_DRAW);
    ...
    function animate() {
        ...
        lightPos = vec3(radius * Math.sin(alpha) + center[0], center[1], radius * Math.cos(alpha) + center[2])
        let d = -(lightPos[1] - (-1))
        let M_p = mat4(vec4(1, 0, 0, 0),
            vec4(0, 1, 0, 0),
            vec4(0, 0, 1, 0),
            vec4(0, 1 / d, 0, 0));
        ...
        gl.uniformMatrix4fv(gl.getUniformLocation(program, "pvt_matrix"), false, flatten(temp))
        gl.drawArrays(gl.TRIANGLES, verteciesPlane.length, verteciesQuad1.length + verteciesQuad2.length);
