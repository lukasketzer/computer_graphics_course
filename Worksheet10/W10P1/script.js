...
window.onload = async () => {
    ...
    function animate() {
        ...
        let R_x = rotate(currentAngle[0], vec3(1.0, 0.0, 0.0))
        let R_y = rotate(currentAngle[1], vec3(0.0, 1.0, 0.0))
        let R = mult(R_x, R_y)
        ...
        if (ready_textures >= total_textures) {
            ...
            // T_shadow = mat4()
            let PVT_shadow = mult(p_shadow, mult(v_shadow, mult(T_shadow, R)))
            gl.uniformMatrix4fv(
